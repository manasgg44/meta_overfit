...
flags.DEFINE_float('noise', 1,
                  'std dev of 0 mean gaussian noise')

flags.DEFINE_bool('retrain', False,
                  'Whether to retrain based on test task')


def train(model, saver, sess, exp_string, data_generator, direction, resume_itr=0):
...
def test(model, saver, sess, exp_string, data_generator, test_num_updates=None):
  ...
  for _ in range(NUM_TEST_POINTS):
    ...
    if model.classification:
      ...
      returning = sess.run(model.returning, feed_dict)
      result = returning['val']
      direction = returning['direction']
      #result = returning

    else:  # this is for sinusoid
      returning = sess.run([model.total_loss1] + model.total_losses2, feed_dict)
      result = returning['val']
      direction = returning['direction']
  ...
  #print(direction[1].shape)
  #print(direction[2].shape)
  #sess.close()
  return direction
  


def main1(direction = 0):
  np.random.seed(0)
  random.seed(0)
  ...
  exp_string = 'cls_' + str(FLAGS.num_classes) + '.mbs_' + str(
      FLAGS.meta_batch_size) + '.ubs_' + str(
          FLAGS.train_update_batch_size) + '.numstep' + str(
              FLAGS.num_updates) + '.updatelr' + str(FLAGS.train_update_lr) + '.noise' + str(FLAGS.noise) 
  ...
  if FLAGS.train:
    ...
    direction = test(model, saver, sess, exp_string, data_generator, test_num_updates)
    #sess.close()
    #FLAGS.retrain = True
    #FLAGS.train = True
    ...
    #sess = tf.InteractiveSession()
    #model.construct_model(
    #    input_tensors=metaval_input_tensors, prefix='metatrain_')
    #train(model, saver, sess, exp_string, data_generator, direction, resume_itr)
    #print("Hello\n")
    #FLAGS.train = False
    #sess = tf.InteractiveSession()
    #model.construct_model(
...
if __name__ == '__main__':
  if FLAGS.train == False :
    ...
    FLAGS.retrain = True
    FLAGS.train = True
    FLAGS.test_set = False
    FLAGS.metatrain_iterations = 20000
    FLAGS.pretrain_iterations = 0
    FLAGS.meta_batch_size = 32
    FLAGS.resume = True
    ##sess.close() 
    ...
    FLAGS.test_set = True
    ##sess.close()
    direction_discarded = main1()        
    
    FLAGS.retrain = True
    FLAGS.train = True
    FLAGS.test_set = False
    FLAGS.metatrain_iterations = 10000
    FLAGS.pretrain_iterations = 0
    FLAGS.meta_batch_size = 32
    FLAGS.resume = True
