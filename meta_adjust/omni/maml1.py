...
class MAML:
  ...
  def construct_model(self, input_tensors=None, prefix='metatrain_'):
    ...
    if input_tensors is None:
      ...
      self.grad_dir = tf.placeholder(tf.float32)
    else:
      self.inputa = input_tensors['inputa']
      self.inputb = input_tensors['inputb']
      self.labela = input_tensors['labela']
      self.labelb = input_tensors['labelb']
      self.grad_dir = input_tensors['grad_dir']
    ...
    with tf.variable_scope('model', reuse=None) as training_scope:
      ...
      def task_metalearn(inp, reuse=True):
        ...
        flattened_grads = tf.constant([], dtype=tf.float32)
        for i in grads :
          aa = tf.reshape(i, [-1])
          flattened_grads = tf.concat([flattened_grads, aa], 0)

        #if FLAGS.retrain and FLAGS.train :
          ##a = tf.nn.l2_normalize(flattened_grads)
          ##b = tf.nn.l2_normalize(self.grad_dir)
          ##c = tf.multiply(a, b)
          ##cos_sim =  tf.reduce_sum(c)
          #cos_sim = tf.reduce_sum(tf.multiply(tf.nn.l2_normalize(flattened_grads), tf.nn.l2_normalize(self.grad_dir)))
          #grads = [tf.multiply(grad, cos_sim) for grad in grads]
        
        self.direction.append(flattened_grads)
        

        #if FLAGS.train:
        #    grads = [grad+ np.random.normal(loc=0.0, scale= FLAGS.noise, size=None) for grad in grads]         
        if FLAGS.stop_grad:
        ...
        for j in range(num_updates - 1):
          ...
          ##if FLAGS.train:
          ##  grads = [grad+ np.random.normal(loc=0.0, scale= FLAGS.noise, size=None) for grad in grads]
          #if FLAGS.train and FLAGS.retrain :
          #  grads = [tf.multiply(grad, cos_sim) for grad in grads]
          if FLAGS.stop_grad:
            grads = [tf.stop_gradient(grad) for grad in grads]
        ...
        if self.classification:
          task_accuracya = tf.keras.metrics.Accuracy(
    ...
    if 'train' in prefix:
      ...
      if FLAGS.metatrain_iterations > 0:
        ...
        if FLAGS.retrain and FLAGS.train :
          ...
          for i in range(len(gvs)-4) :
            aa = tf.reshape(gvs[i][1], [-1])
            flattened_grads = tf.concat([flattened_grads, aa], 0)
          a = tf.nn.l2_normalize(flattened_grads)
          b = tf.nn.l2_normalize(self.grad_dir)
          c = tf.multiply(a, b)
          cos_sim =  tf.reduce_sum(c)
          gvs = [(tf.multiply(grad, cos_sim), var) for grad,var in gvs]
      ...
      if self.classification:
        ...
        #self.returning = [self.metaval_total_accuracy1] + self.metaval_total_accuracies2
        self.returning = {'val' : [self.metaval_total_accuracy1] + self.metaval_total_accuracies2, 'direction': self.direction[0]}
  ...
  def construct_conv_weights(self):
    ...
    dtype = tf.float32
    conv_initializer = tf.keras.initializers.GlorotNormal(dtype=dtype)
    fc_initializer = tf.keras.initializers.GlorotNormal(dtype=dtype)
    k = 3
